...
func BenchmarkSeekPrefixCombined(b *testing.B) {
	wordCount := 100000
	words := make([]string, 0)
	words = append(words, "")
	...
	if len(resultsImmutable) != len(resultsOptimized) {
		b.Fatalf("expected %d, got %d", len(resultsImmutable), len(resultsOptimized))
	}
	// assert both slices are equal
	if !reflect.DeepEqual(resultsImmutable, resultsOptimized) {
		b.Fatalf("expected %s, got %s", resultsImmutable, resultsOptimized)
	}
}